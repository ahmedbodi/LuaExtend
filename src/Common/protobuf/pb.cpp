...
{
#ifdef IS_LITTLE_ENDIAN
	memcpy(cache, buffer, sizeof(int32_t));
    return cache;
#else
...
{
#ifdef IS_LITTLE_ENDIAN
	memcpy(cache, buffer, sizeof(int64_t));
    return cache;
#else
...
static int struct_unpack(lua_State *L)
{
    ...
    switch(format){
        case 'i':
            {
                lua_pushinteger(L, *(int32_t*)unpack_fixed32(buffer, (uint8_t*)out));
                break;
            }
        case 'q':
            {
                lua_pushnumber(L, (lua_Number)*(int64_t*)unpack_fixed64(buffer, (uint8_t*)out));
                break;
            }
        case 'f':
            {
                lua_pushnumber(L, (lua_Number)*(float*)unpack_fixed32(buffer, (uint8_t*)out));
                break;
            }
        case 'd':
            {
                lua_pushnumber(L, (lua_Number)*(double*)unpack_fixed64(buffer, (uint8_t*)out));
                break;
            }
        case 'I':
            {
                lua_pushnumber(L, *(uint32_t*)unpack_fixed32(buffer, (uint8_t*)out));
                break;
            }
        case 'Q':
            {
                lua_pushnumber(L, (lua_Number)*(uint64_t*)unpack_fixed64(buffer, (uint8_t*)out));
                break;
            }
        default:
            luaL_error(L, "Unknown, format");
