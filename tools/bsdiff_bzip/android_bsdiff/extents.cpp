...
const char* read_llong(const char* s,
  ...
  const char* next_s;
  errno = 0;
  INT64 val = strtoll(s, (char**)&next_s, 10);
 ...
 * was found. */
static int extents_read(const char* ex_str, ex_t* ex_arr, size_t ex_count) {
  size_t i;
...
bool ParseExtentStr(const char* ex_str, std::vector<ex_t>* extents) {
  ...
  int ret = extents_read(ex_str, NULL, 0);
  if (ret < 0)
